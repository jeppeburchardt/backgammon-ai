...
function TestA(id) {
    ...
    function applyScoreToPermutation(p) {
        ...
        if (endgame) {
            ...
            //everybodys home
            score += (23 - lowestPosition) > 6 ? 18 : 0;

            var tilesOccupied = (23 - lowestPosition) > 6 ? (23 - lowestPosition) : 6;
            ...
            p.board.players[self.id].checkers.forEach(function (numCheckers, tile) {
                if (tile > 20) {
                    score -= Math.abs(3 - numCheckers);;
                } else if (tile === 20) {
                    score -= Math.abs(3 - numCheckers);
                }
                else if (tile === 19) {
                    score -= Math.abs(5 - numCheckers);
                }
                else if (tile === 18) {
                    score -= Math.abs(7 - numCheckers);
                } else {
