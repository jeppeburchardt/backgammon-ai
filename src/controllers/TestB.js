...
function TestB(id) {
    ...
    function applyScoreToPermutation(p) {
        ...
        p.score = getScore(p, self.id) - getScore(p, 1 - self.id);

    }
    function getScore(p, playerId) {
        var opponent = 1 - playerId;
        var lowestPositionOfOpponent = self.lowestPositionOfOpponent(p.board, opponent);
        var lowestPosition = self.lowestPositionOfSelf(p.board, playerId);
        var checkersOnBoard = (15 - p.board.players[playerId].bearedOff);
        var pip = getPip(p.board.players[playerId].checkers);
        ...
        if (endgame) {
            ...
            p.board.players[playerId].checkers.forEach(function (numCheckers, tile) {
                if (tile >= 18) {
            ...
            score += p.board.players[playerId].bearedOff * 12;
            ...
            p.board.players[playerId].checkers.forEach(function (numCheckers, tile) {
                ...
                if (numCheckers >= 2 && tile > 0 && p.board.players[playerId].checkers[tile - 1] >= 2) {
                    score += 1;
                }

            });
            // Priming a long chain of blocked tiles is good
            if (p.board.players[playerId].checkers[23] >= 2 && p.board.players[playerId].checkers[24] >= 2) {
                score += 1;
            }

            if (p.board.players[playerId].checkers[16] > 2) {
                score += 1;
            }
            if (p.board.players[playerId].checkers[17] > 2) {
                score += 1;
            }
            if (p.board.players[playerId].checkers[18] > 2) {
                score += 1;
            }
        }
    ...
    self.lowestPositionOfOpponent = function (board, playerid) {
        var lowestPositionOfOpponent = getlowestPosition(board.players[playerid].checkers);
        // reverse direction:
        lowestPositionOfOpponent = 23 - lowestPositionOfOpponent;

        return board.players[1 - self.id].hits > 0 ? 24 : lowestPositionOfOpponent;
    };
    self.lowestPositionOfSelf = function (board, playerid) {
        var lowestPositionOfSelf = getlowestPosition(board.players[playerid].checkers);
